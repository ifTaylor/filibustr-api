...
from filibustr_api.models.bill_model import Bill, Sponsor
from ..database.bill_orm import BillORM
from ..database.sponsor_orm import SponsorORM
from sqlalchemy.orm import selectinload

async def get_bills_from_db(session: AsyncSession, limit: int, offset: int) -> list[Bill]:
    stmt = (
        select(BillORM)
    ...
    return [
        Bill(
            ...
            ) if r.sponsor else None
        )
        for r in records
    ]

async def save_bill(session: AsyncSession, bill: Bill) -> None:
